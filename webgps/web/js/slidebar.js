...
SlideBar.setValue = function(id,evt){
	...
	if(typeof v != "object"){
		...
		id.style.left = L + "px";
		id.value = (L*id.handle.maxValue/(id.parentNode.clientWidth-id.clientWidth-id.getStyle("border-left-width").cint()*2)).toFixed(0);
	}
...
SlideBar.moveCursor = function(evt){
	...
	function move(evt){
		...
		me.style.left = w + "px";
		me.value = ((w/meWidth)*me.handle.maxValue).toFixed(0);
