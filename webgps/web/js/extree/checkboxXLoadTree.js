...
function WebFXCheckBoxLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6);
...
function WebFXLoadCheckBoxTreeItem(sText, sValue, sXmlSrc, eParent, sIcon, sOpenIcon, bChecked, disabled, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6) {
	// call super	
	...
	this.base(sText, sValue, eParent, sIcon, sOpenIcon, bChecked, disabled, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6);
...
function checkbox_createNode(oNode, parentDisabled, parentChecked){
	// retreive attributes
	...
	var isOndblclick = oNode.getAttribute("isOndblclick");

	var sId = oNode.getAttribute("id");
	var sDesc = oNode.getAttribute("desc");
	var sExpend1 = oNode.getAttribute("expend1");
	var sExpend2 = oNode.getAttribute("expend2");
	var sExpend3 = oNode.getAttribute("expend3");
	var sExpend4 = oNode.getAttribute("expend4");
	var sExpend5 = oNode.getAttribute("expend5");
	var sExpend6 = oNode.getAttribute("expend6");
	
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadCheckBoxTreeItem(text, value, src, parent, icon, openIcon, checked, disabled, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6);
	else{
		jsNode = new WebFXCheckBoxTreeItem(text, value, parent, icon, openIcon, checked, disabled, sId, sDesc, sExpend1, sExpend2, sExpend3, sExpend4, sExpend5, sExpend6);
    	if(action!=null){
    		jsNode.setAction(action);
    	}
    	if(isOndblclick!=null && isOndblclick=="true"){
    		jsNode.setOndblclick(true);
    	}
    }
    // 将选中的值加入结果集中.
	if(checked){
		//checkedValues.put(jsNode.id,  new CheckedObject(jsNode.id, text, value));  // 这里的id是系统自动生成的id，不是oid（业务对象id）。
		checkedValues.put(jsNode.id,  jsNode);  // 这里的id是系统自动生成的id，不是oid（业务对象id）。
	}
...
function checkbox_xmlTreeToJsTree(oNode, parentDisabled, parentChecked) {	
	// create jsNode
	var jsNode;  
	var type = oNode.getAttribute("type");
	if(type != null && type != ""){
		jsNode = _createNodeByType(type, oNode, parentDisabled, parentChecked);	
	}else{
		jsNode = checkbox_createNode(oNode, parentDisabled, parentChecked);
	}
	...
	for (var i = 0; i < l; i++) {
		if (cs[i].tagName == "tree"){
			var disabled = _getDisabled_A(oNode, parentDisabled);			
			jsNode.add( checkbox_xmlTreeToJsTree(cs[i], disabled, parentChecked), true );
		}
	}
		...
		for (var i = 0; i < l; i++) {
			if (cs[i].tagName == "tree") {
				...
				if(webFXTreeConfig.cascadeCheck){
					jsParentNode.add( checkbox_xmlTreeToJsTree(cs[i], jsParentNode._disabled, jsParentNode.getChecked()), true);
				}else{
