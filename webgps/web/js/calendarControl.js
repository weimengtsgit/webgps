   ...
   //月、时、分选择下拉框
   var s_tiannet_select2 = "width:50px;display:none;";
   //日期选择控件体的样式
   var s_tiannet_body = "width:180;background-color:#2650A6;display:none;z-index:9998;position:absolute;" +
     "border-left:1 solid #CCCCCC;border-top:1 solid #CCCCCC;border-right:1 solid #999999;border-bottom:1 solid #999999;";
   //显示日的td的样式
   var s_tiannet_day = "width:25px;height:20px;background-color:#D8F0FC;font-size:10pt;";
   //字体样式
   ...
   for(var i =0;i < weekName.length;i ++){
    //输出星期
    document.write('<td width="26" align="center" Author="tiannet">' + weekName[i] + '</td>');
   }
   document.write(' </tr>');
   ...
   function tiannetShowControl(){
    ...
    //tiannetHideObject("Select");//轨迹回放页面受到影响，暂时屏蔽对控件的隐藏 modify by yanglei；

    //tiannetHideObject("OBJECT");//因会把地图Map隐藏掉

   }
   //根据标签名称隐藏标签。如会遮住控件的select，object
   function tiannetHideObject(strTagName) {
    
   var x = document.all.divTiannetDate.offsetLeft;
   var y = document.all.divTiannetDate.offsetTop;
   var h = document.all.divTiannetDate.offsetHeight;
   var w = document.all.divTiannetDate.offsetWidth;
