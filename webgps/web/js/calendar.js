...
function calendar(obj_target) {
	...
	this.pop_direction = 1;	//1:右下角,2:右上角
...
function cal_popup3 (str_datetime) {
	...
	var left = 300;
	if( event != null ){
		top = event.screenY+document.body.scrollTop;
		if( 2 == this.pop_direction ){
			top -= (this.time_comp ? 215 : 190) + 28;
		}
	...
	var obj_calwindow = window.open(
		'../js/calendar.html?id=' + this.id + '&s=' + this.dt_selected.valueOf() + '&c=' + this.dt_current.valueOf(),
...
function cal_prs_time3 (str_time, dt_date) {
	...
	if (!arr_time[0]) dt_date.setHours(0);
		...
		else return cal_error ("无效的小时");
	else return cal_error ("无效的小时");
	
	if (!arr_time[1]) dt_date.setMinutes(0);
	else if (RE_NUM.exec(arr_time[1]))
		if (arr_time[1] < 60) dt_date.setMinutes(arr_time[1]);
		else return cal_error ("无效的分钟");
	else return cal_error ("无效的分钟");

	if (!arr_time[2]) dt_date.setSeconds(0);
	else if (RE_NUM.exec(arr_time[2]))
		if (arr_time[2] < 60) dt_date.setSeconds(arr_time[2]);
		else return cal_error ("无效的秒");
	else return cal_error ("无效的秒");
