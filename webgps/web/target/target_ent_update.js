...
var proxy = new Ext.data.HttpProxy({
	url : path + '/target/target.do?method=getEntTarget'
});
				...
				new Ext.Action({
			        ...
			        handler: function(){
						...
						Ext.each(modified, function(item) {
							if (json == '') {
								json = item.data.id + ',' + item.data.billAmount + ',' + item.data.cashAmount + ',' + item.data.costAmount + ',' + item.data.visitAmount + ',' + item.data.cusVisitAmount + ',' + item.data.targetOn + ',' + item.data.year;
							}else{
								json += ';' + item.data.id + ',' + item.data.billAmount + ',' + item.data.cashAmount + ',' + item.data.costAmount + ',' + item.data.visitAmount + ',' + item.data.cusVisitAmount + ',' + item.data.targetOn + ',' + item.data.year;
							}
						});
						if (json != '') {
							...
							Ext.Ajax.request({
								url: path + '/target/target.do?method=updateEntTarget',
								params: { data: json },
								timeout:180000,
								method: 'POST',
