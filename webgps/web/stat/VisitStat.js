...
function showgriddetail(record){
	...
	Ext.getCmp('griddetail_title').setText('('+tmpv+')的拜访统计报表:');
	storedetail.load({params:{start:0, limit:14,startTime: startTime, endTime: endTime, deviceId: deviceid,duration: duration}});
}
    ...
    listeners:{
    	beforeload:{
    		fn: function(thiz,options){
    			this.baseParams ={
    				startTime: startTime, endTime: endTime, deviceId : deviceid,duration: duration
    			}
    		}
    	}
    }
...
var proxy = new Ext.data.HttpProxy({
    url: path+'/stat/visitStat.do?method=listVisitCountTj'
});
...
var grid = new Ext.grid.GridPanel({
    ...
    columns: [
            {id:'id',header: 'id', width: 10, sortable: true, dataIndex: 'id', hidden:true},
            {header: '业务员名称', width: 130, sortable: true,  dataIndex: 'vehicleNumber'},
			{header: '拜访次数', width: 130, sortable: true,  dataIndex: 'visitCount'},
			{id: 'process',header: '操作', width: 100, sortable: true,  dataIndex: 'process' , renderer: function (value, meta, record) {   
        ...
        ,new Ext.form.TextField({
	        ...
	        handler: function(){
    			...
    			document.excelform.action = path+'/stat/visitStat.do?method=listVisitCountTj&'+tmpparam;
