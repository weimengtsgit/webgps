...
function _delete(myform, doAction){
...
function deleteAll(myform, forward, clazz){
	...
	if(result.length>0){
        ...
        if(!confirm("确定删除这些记录吗？")) return;
	myform.clazz.value=clazz;
	//alert(myform.clazz.value);
	//myform.ids.value=result;
	myform.forward.value=forward;
    //hxy replace 06/01/10
	myform.action='/List.do?action=deleteAll';
    //alert("myform.action="+myform.action);
    //hxy end
	//window.location.href='List.do?action=deleteAll&forward=' + forward
	//						+ '&clazz='+ clazz
	//						+ '&ids=' + result;
	/*
	openDefaultWindow('List.do?action=deleteAll&forward=' + forward
							+ '&clazz='+ clazz
							+ '&ids=' + result);
	*/
	myform.submit();
