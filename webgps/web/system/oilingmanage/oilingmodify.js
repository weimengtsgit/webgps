	...
	handler: function(){
		...
		Ext.MessageBox.confirm('提示', '您确定要修改数据吗?', function (btn){
			if(btn=='yes'){
				...
				Ext.Ajax.request({
					url:path+'/oiling/oiling.do?method=updateOiling',
					method :'POST', 
					params:{
						id: tmpid, deviceId: tmpdeviceId, oilLiter: tmpoilLiter, oilCost: tmpoilCost,
...
var sm = new Ext.grid.RowSelectionModel({
	...
	listeners: {
		rowselect: function(sm, row, rec) {
			...
			Ext.getCmp('frmoilLiter').setValue(rec.get('oilLiter'));
			Ext.getCmp('frmoilCost').setValue(rec.get('oilCost'));
