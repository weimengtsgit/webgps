	...
	handler: function(){
		Ext.MessageBox.confirm('提示', '您确定要添加新数据吗?', function (btn){
			if(btn=='yes'){
				...
				var tmpmaintenanceDate = Ext.getCmp('frmmaintenanceDate').getValue();
				if(tmpmaintenanceDate.length<=0){
					Ext.Msg.alert('提示', '请选择维护日期!');
					return;
				}
				var tmpexpenses = Ext.getCmp('frmexpenses').getValue();
				if(tmpexpenses.length<=0){
					Ext.Msg.alert('提示', '请输入维护费用!');
					return;
				}
				var tmpcondition = Ext.getCmp('frmcondition').getValue();
				if(tmpcondition.length<=0){
					Ext.Msg.alert('提示', '请输入维护情况!');
					return;
				}
				...
				Ext.Ajax.request({
					url:path+'/vehiclesMaintenance/vehiclesMaintenance.do?method=addVehiclesMaintenance',
					method :'POST', 
					params:{
						deviceId: encodeURI(tmpdeviceId), maintenanceDate: tmpmaintenanceDate , expenses:encodeURI(tmpexpenses) ,
						condition: encodeURI(tmpcondition), handler: encodeURI(tmphandler), expireDate: tmpexpireDate ,
...
var userColumns =  [
    ...
    {header: "维护费用",fixed:true, width: 100, sortable: true, dataIndex: 'expenses'},
