 ...
 * @author: zhangwei
 * @version 1.0
 * @date: 2008-8-30 下午02:41:11
 */

public class PagedRSEx extends PagedRS {
    public PagedRSEx() //construct function
    {
    }
...
//    }
    ...
    public String getHtmlPageLink() {
        String sHtml = "<table width=100% border=0 cellspacing=0 cellpadding=0 align=center height=30>" 
         ...
         /*   sHtml += "<td>" 
	               ...
	               + "\" style=cursor:hand onmouseover=\"this.className='spanbutton_over'\" onmousedown=\"this.className='spanbutton_down'\" onmouseup=\"this.className='spanbutton_up'\" onmouseout=\"this.className='spanbutton_normal'\"><a href='javascript:window.location.href=window.location.href ;'><img src=../images/refresh.gif align=adsmiddle border=0>刷新</a></span></td>" 
                   ;*/
     ...
     * @param sDefaultGroupby
     * @param abIsOrderField
     * @param asPropCnNames
     * @param asRequiredProperties
     * @param sLink
     * @param sLinkViewForm
     * @param sLinkCancelForm
     * @param sCnFormName
     * @param sCnDefaultOrderBy
     */
    public void init(javax.servlet.http.HttpServletRequest request,
            String sQuerySql, String sDefaultOrderBy,String sDefaultGroupby, boolean[] abIsOrderField,
            String[] asPropCnNames, String[] asRequiredProperties,
            String sLink, String sLinkViewForm, String sLinkCancelForm,
            String sCnFormName, String sCnDefaultOrderBy) {
        ...
        this.sQuerySql = sQuerySql;
        this.sDefaultOrderBy = sDefaultOrderBy;
        this.sDefaultGroupby = sDefaultGroupby;
        ...
        if (sOrderBy == null) {
            this.sQuerySql = sQuerySql + sSqlWhere + sDefaultGroupby+" order by "
                    + sDefaultOrderBy;
            sOrderBy = sDefaultOrderBy;
        } else
            this.sQuerySql = sQuerySql + sSqlWhere + sDefaultGroupby+" order by " + sOrderBy;
...
//                	}
                	if(fieldname.toLowerCase().indexOf("create_date") >= 0 || fieldname.toLowerCase().indexOf("lastupdate_date") >= 0){
                		sSqlWhere += " and " + fieldname + " between    to_date('"
                        + sStartDate + "', 'yyyy-mm-dd') " 
                        + " and  to_date('" + sEndDate
                        + "', 'yyyy-mm-dd')  ";
                	}else{
                		sSqlWhere += " and " + fieldname + " between to_date('"
                        + sStartDate + "', 'yyyy-mm-dd') " 
